...
> - 实现 `WeChatMessageReader`：聊天页内容变化扫描消息节点（contentDescription "发送者 说：内容" 解析）→ 去重 → 复用系统 TTS/内置 Matcha 语音层播报；微信通知到达时兜底播报（后台收消息场景）。
> - 播报不干扰微信拨号自动化（`WeChatData.index != 0` 时跳过）；无障碍事件配置增加 `typeWindowContentChanged`。
> - 设置页新增「微信消息点读」开关（默认关闭，隐私优先）。
> - 验收步骤（用户）：系统设置开启 MoreTalk 无障碍服务 → 设置页打开「微信消息点读」→ 让家属发一条微信消息 → 应听到"新消息，XX说：内容"。
